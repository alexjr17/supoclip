...
class VideoService:
    ...
    @staticmethod
    def _clip_budget_for_duration(duration_seconds: float) -> tuple[int, int]:
        """Return (min, max) clips based on video length.

        Clips are not forced: short videos yield fewer clips, longer videos
        yield more. Bands are approximate so small length differences do not
        cause jitter.
        """
        minutes = max(1.0, (duration_seconds or 0.0) / 60.0)
        if minutes < 6:
            return (2, 4)
        if minutes < 12:
            return (3, 5)
    ...
    async def process_video_complete(
        ...
        try:
            ...
            # Clip budget scales with video length; nothing is forced.
            min_segments, max_segments = VideoService._clip_budget_for_duration(
                file_duration or 0.0
            )
            ...
            elif add_subtitles and not load_cached_transcript_data(video_path):
                # A cached transcript in the DB provides only formatted text, not
                # the word-level timing burned captions need. If the on-disk word
                # cache is missing, re-run transcription so subtitles (and their
                # emojis) actually render. Fall back to the cached text on failure.
                logger.info(
                    "Cached transcript present but word-level cache missing; "
                    "regenerating transcript for subtitle timing"
                )
                try:
                    transcript = await VideoService.generate_transcript(
                        video_path, processing_mode=processing_mode
                    ...
                    logger.warning(
                        "Transcript regeneration failed (%s); using cached text", exc
                    )
            ...
            segments_json = segments_json[:max_segments]

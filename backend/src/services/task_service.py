...
from .video_service import VideoService, UPLOAD_URL_PREFIX
...
class TaskService:
    ...
    async def process_task(
        ...
        try:
            ...
            # Clear any clips left behind by a previous (retried) attempt so
            # arq retries never duplicate rows or files for the same task.
    ...
    async def delete_task(self, task_id: str) -> None:
        """Delete a task, its clips, and all associated files from disk."""
        task = await self.task_repo.get_task_by_id(self.db, task_id)

        clips = await self.clip_repo.get_clips_by_task(self.db, task_id)
        clip_paths = [Path(c["file_path"]) for c in clips if c.get("file_path")]

        source_video_paths: list[Path] = []
        if task:
            source_url = task.get("source_url")
            if source_url:
                for cache in await self.cache_repo.get_caches_by_source_url(
                    self.db, source_url
                ):
                    if cache.get("video_path"):
                        source_video_paths.append(Path(cache["video_path"]))
                if source_url.startswith(UPLOAD_URL_PREFIX):
                    uploads_filename = Path(
                        source_url.removeprefix(UPLOAD_URL_PREFIX)
                    ).name
                    source_video_paths.append(
                        Path(self.config.temp_dir) / "uploads" / uploads_filename
                    )
        source_video_paths = list({str(p): p for p in source_video_paths}.values())

        # Delete DB rows first
        await self.clip_repo.delete_clips_by_task(self.db, task_id)
        await self.task_repo.delete_task(self.db, task_id)

        # Best-effort cleanup of files left on disk
        for clip_path in clip_paths:
            self._delete_file(clip_path)
        for video_path in source_video_paths:
            self._delete_file(video_path)
            self._delete_file(video_path.with_suffix(".assemblyai.mp3"))
            self._delete_file(video_path.with_suffix(".transcript_cache.json"))
        self._delete_export_files()

        logger.info(
            f"Deleted task {task_id} and all associated clips and source files"
        )
    ...
    def _delete_export_files(self) -> None:
        exports_dir = Path(self.config.temp_dir) / "exports"
        try:
            ...
            logger.warning(f"Could not clear exports directory {exports_dir}: {e}")
    ...
    async def regenerate_all_clips_for_task(
        ...
        add_subtitles = (
            ...
            else metadata.get("add_subtitles", True)
        )
        show_emojis = (
            show_emojis if show_emojis is not None else metadata.get("show_emojis")
        )
    ...
    async def split_clip(
        ...
        first_path, second_path = await asyncio.to_thread(
            split_clip_file,
            input_path,
            Path(self.config.temp_dir) / "clips",
            split_time,
    ...
    async def update_clip_captions(
        ...
        task_settings = await self._load_task_source_settings(task_id)
        show_emojis = task_settings.get("show_emojis")
    ...
    def _get_clip_source_ranges(clip: Dict[str, Any]) -> list[tuple[float, float]]:
        ...
        if isinstance(duration, (int, float)) and duration > 0.05:
            return [(start_seconds, start_seconds + float(duration))]
        return []
    ...
    async def _load_task_source_settings(self, task_id: str) -> Dict[str, Any]:
        ...
        show_emojis = parsed.get("show_emojis", defaults["show_emojis"])
        if not isinstance(show_emojis, bool):
            show_emojis = defaults["show_emojis"]

...
from .caption_font_metrics import (
    CaptionLayout,
    measure_caption_line,
    resolve_caption_font_path,
    shape_text,
)
from .caption_font_metrics import effective_font_size as libass_effective_size
...
def _assemblyai_speech_model_value(speech_model: str) -> str:
    normalized = (speech_model or "universal").strip().lower()
    if normalized in {"universal-2"}:
        return "universal-2"
    if normalized in {"universal-3-5-pro", "universal-3-pro", "best"}:
...
def emoji_rendering_supported() -> bool:
    """Whether this environment's libass rasterises emoji glyphs (cached, one-shot).

    Caption emojis are only injected when this returns True, so we never burn
    ugly ".notdef" tofu boxes. The probe burns a single emoji (written as raw
    UTF-8 and rendered with the monochrome "Noto Emoji" font) and checks the
    glyph renders as a filled shape. libass in this environment cannot
    rasterise colour emoji layers (CBDT/COLR/SVG), so monochrome white glyphs
    are the accepted rendering.
    """
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="supoclip_emojiprobe_") as probe_dir:
            ...
            if run_ffmpeg_command(command, timeout=60).returncode == 0 and frame.exists():
                ...
                arr = np.asarray(Image.open(frame).convert("L"), dtype=np.int16)
                ink = arr > 40
                # Tofu boxes and glyphs the renderer skips leave too little (or no)
                # ink. Require a solid filled shape, not a hollow box, by checking
                # the ink ratio inside the glyph's bounding box.
                if ink.sum() >= 300:
                    ys, xs = np.where(ink)
                    bbox_h = ys.max() - ys.min() + 1
                    bbox_w = xs.max() - xs.min() + 1
                    if bbox_h * bbox_w > 0:
                        result = (ink.sum() / (bbox_h * bbox_w)) > 0.25
    except Exception as exc:
        logger.info("Emoji support probe failed (%s); disabling caption emojis", exc)
        result = False

    _EMOJI_SUPPORT_CACHE = result
    logger.info("Caption emoji rendering supported: %s", result)
    return result
...
def ass_escape_emoji(value: str) -> str:
    """Return the emoji as raw UTF-8 text for the ASS dialogue line.

    The emoji run is always rendered with the monochrome ``EMOJI_FONT_NAME``
    override, so the raw character bytes rasterise as a glyph. The legacy
    \\UXXXXXXXX escape form must NOT be used: libass in this environment
    renders those escapes as their literal text (e.g. "\\U0001F3D8") instead
    of decoding them.
    """
    return str(value)
...
def build_assemblyai_ass_subtitles(
    ...
    # A user-provided show_emojis toggle overrides the template's own emoji flag.
    template_emoji = bool(template.get("emoji", True))
    emoji_enabled = show_emojis if show_emojis is not None else template_emoji
    enable_emoji = bool(emoji_enabled) and emoji_rendering_supported()
    ...
    # Emoji-above layout: each contextual emoji is emitted as its own ASS event
    # positioned over its word. The caption line is measured the way libass lays
    # it out (effective font size, advance-based pens, advance-centred line).
    measure_path = (
        resolve_caption_font_path(effective_font_family, font_name)
        if enable_emoji
        else None
    )
    emoji_path = (
        resolve_caption_font_path(EMOJI_FONT_NAME, EMOJI_FONT_NAME)
        if enable_emoji
        else None
    )
    EMOJI_SCALE = 1.5
    emoji_fs = int(font_px * EMOJI_SCALE)
    emoji_eff_px = (
        libass_effective_size(emoji_fs, emoji_path) if emoji_path is not None else emoji_fs
    )
    border_style = (
        3
...
"""
    ...
    def display_text(word: Dict[str, Any]) -> str:
        text = str(word.get("text", ""))
        if uppercase:
            text = text.upper()
        return text
    ...
    def idle_span(global_idx: int, disp: str, has_emoji: bool = False) -> str:
        color = (
            primary
            if (has_emoji or not enable_emphasis or global_idx not in emphasis_idx)
            else emphasis_color
        )
    ...
    def emoji_event(start: float, end: float, x: int, y: int, emoji_char: str) -> str:
        return (
            f"Dialogue: 0,{ass_timestamp(start)},{ass_timestamp(end)},Default,,0,0,0,,"
            f"{{\\pos({x},{y})}}"
            f"{{\\fs{emoji_fs}\\fscx92\\fscy92\\t(0,140,\\fscx130\\fscy130)"
            f"\\t(140,260,\\fscx100\\fscy100)\\fad(0,180)}}"
            f"{{\\fn{EMOJI_FONT_NAME}}}{ass_escape_emoji(emoji_char)}"
        )

    def emoji_position(layout: CaptionLayout, emoji_char: str) -> tuple[int, int]:
        # Centered on the whole visible caption line (which itself is centred
        # on the video), not over an individual word.
        x = video_width // 2
        emoji_shape = (
            shape_text(emoji_char, emoji_path, emoji_eff_px) if emoji_path is not None else None
        )
        if emoji_shape is None:
            return video_width // 2, int(y_pos - font_px)
        baseline_abs = y_pos + (layout.ascender + layout.descender) / 2
        text_ink_top_abs = baseline_abs + layout.line_ink_top
        gap = max(4, 0.1 * layout.size_px)
        emoji_ink_h = emoji_shape.ink_bottom - emoji_shape.ink_top
        desired_center = text_ink_top_abs - gap - emoji_ink_h / 2
        y = desired_center + (emoji_shape.advance_center_y - emoji_shape.ink_center_y)
    ...
    for chunk_start in range(0, total, chunk_size):
        ...
        layout = (
            measure_caption_line([display_text(w) for w in chunk], measure_path, font_px)
            if enable_emoji and measure_path is not None
            else None
        )
        ...
        if animation == "karaoke":
            for local_i, word in enumerate(chunk):
                ...
                for local_j, other in enumerate(chunk):
                    ...
                    has_emoji = bool(emoji_by_idx.get(gj))
                    parts.append(
                        active_span(disp)
                        if local_j == local_i
                        else idle_span(gj, disp, has_emoji)
                    )
                line = " ".join(parts)
            ...
            for local_j, word in enumerate(chunk):
                ...
                color = (
                    primary
                    if (has_emoji or not enable_emphasis or gj not in emphasis_idx)
                    else emphasis_color
                )
        ...
        if enable_emoji:
            for local_i, word in enumerate(chunk):
                ...
                emoji_char = emoji_by_idx.get(global_idx)
                if not emoji_char:
                    continue
                if animation == "karaoke":
                    start = float(word["start"])
                ...
                if layout is not None:
                    x, y = emoji_position(layout, emoji_char)
                else:
                    x, y = video_width // 2, int(y_pos - font_px)
                events.append(emoji_event(start, end, x, y, emoji_char))
...
def build_speaker_timeline_from_transcript(
    transcript_data: Optional[Dict],
    keep_ranges: List[Tuple[float, float]],
) -> List[Dict[str, Any]]:
    """Project diarized speaker utterances onto the clip output timeline.

    Uses the real speaker labels from the transcript cache (AssemblyAI
    ``speaker_labels=True``) instead of inferring the active speaker from
    motion. The projection applies the same crossfade shortening as
    ``get_words_for_keep_ranges`` so the pan stays locked to the spoken audio.
    """
    if not transcript_data or not keep_ranges:
        return []

    utterances = transcript_data.get("utterances") or []
    speaker_labels = {
        utterance.get("speaker") for utterance in utterances if utterance.get("speaker")
    }
    ...
    for index, (keep_start, keep_end) in enumerate(keep_ranges):
        ...
        for utterance in utterances:
            ...
            if not speaker:
                continue
            utt_start = utterance["start"] / 1000.0  # ms -> s (source timeline)
            utt_end = utterance["end"] / 1000.0
            overlap_start = max(utt_start, keep_start)
            overlap_end = min(utt_end, keep_end)
            if overlap_end <= overlap_start:
                continue
            segments.append(
                {
                    "start": timeline_offset + (overlap_start - keep_start),
                    "end": timeline_offset + (overlap_end - keep_start),
                    "speaker": speaker,
                }
            )
    ...
    for segment in segments:
        if (
            ...
            and segment["start"] <= merged[-1]["end"] + 0.15
        ):
...
def assign_speaker_sides(
    timeline: List[Dict[str, Any]],
    times: List[float],
    left_values: List[float],
    right_values: List[float],
) -> Dict[str, str]:
    """Map diarized speaker labels to physical left/right using motion.

    The diarization tells us *when* each label speaks but not *which side of
    the frame* that label sits on. We correlate the already-computed motion
    signal with each label's speaking windows: over all its turns, the side
    with more frame motion is assumed to be that speaker's side. Returning an
    empty dict means the signal is too weak to trust, and callers fall back to
    motion-only inference.
    """
    sides: Dict[str, str] = {}
    if (
        not timeline
    ...
    for segment in timeline:
        ...
        mask = (times_array >= segment["start"]) & (times_array <= segment["end"])
        sample_count = int(np.sum(mask))
        if sample_count < 2:
            continue
        if speaker not in accumulated:
            accumulated[speaker] = [0.0, 0.0, 0]
        accumulated[speaker][0] += float(np.sum(left_array[mask]))
        accumulated[speaker][1] += float(np.sum(right_array[mask]))
        accumulated[speaker][2] += sample_count

    for speaker, (left_sum, right_sum, samples) in accumulated.items():
        if samples < 4:
            continue
        sides[speaker] = "left" if left_sum >= right_sum else "right"
    return sides
...
def detect_speaker_reframe_plan(
    ...
    Prefers real diarized speech timing (``transcript_data`` + ``keep_ranges``)
    over motion heuristics. Motion is still sampled so we can map each
    diarized speaker label to its physical left/right side; when diarization is
    unavailable or too weak, we fall back to pure motion inference.
    """
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="supoclip_motion_") as motion_dir:
            ...
            timeline: List[Dict[str, Any]] = []
            diarized = build_speaker_timeline_from_transcript(
                transcript_data, keep_ranges or []
            )
            if len(diarized) >= 2:
                sides = assign_speaker_sides(
                    diarized, times, left_values, right_values
                )
                if len(sides) >= 2:
                    side_timeline: List[Dict[str, Any]] = []
                    for segment in diarized:
                        side = sides.get(segment["speaker"])
                        if not side:
                            continue
                        if (
                            side_timeline
                            and side_timeline[-1]["speaker"] == side
                            and segment["start"]
                            <= side_timeline[-1]["end"] + 0.15
                        ):
                            side_timeline[-1]["end"] = max(
                                side_timeline[-1]["end"], segment["end"]
                            )
                            continue
                        side_timeline.append(
                            {
                                "start": segment["start"],
                                "end": segment["end"],
                                "speaker": side,
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="supoclip_render_") as temp_dir:
            ...
            framed_ok, _, _ = render_reframed_clip_ffmpeg(
                ...
                fonts_dir=fonts_dir,
                transcript_data=load_cached_transcript_data(video_path),
                keep_ranges=effective_keep_ranges,
            )
            if not framed_ok:

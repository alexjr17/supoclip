...
def _merge_task_source_metadata(
    ...
    *,
    ...
    if isinstance(show_emojis, bool):
        merged["show_emojis"] = show_emojis
    if cleanup_settings:
...
async def create_task(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not isinstance(add_subtitles, bool):
        add_subtitles = True
    show_emojis = data.get("show_emojis")
    ...
    try:
        ...
        previous_usage = await task_service.task_repo.find_previous_source_usage(
        ...
        if previous_usage:
            response["warning"] = {
                ...
                "message": (
                    "Este video ya fue procesado antes. Los clips nuevos se "
                    "generarán de nuevo desde el mismo video."
                ),
...
async def delete_clip(
    ...
    try:
        ...
        if file_path:
            try:
                clip_file = Path(file_path)
                if clip_file.exists():
                    clip_file.unlink()
            except OSError as e:
                logger.warning(f"Could not delete clip file {file_path}: {e}")
...
async def apply_task_settings(
    ...
    try:
        ...
        add_subtitles = payload.get("add_subtitles")
        if not isinstance(add_subtitles, bool):
            add_subtitles = None
        show_emojis = payload.get("show_emojis")
        if not isinstance(show_emojis, bool):
            show_emojis = None
        cleanup_settings = normalize_clip_cleanup_settings(
        ...
        await _save_task_source_metadata(
            ...
            _merge_task_source_metadata(
                ...
                show_emojis=(
                    show_emojis
                    if isinstance(show_emojis, bool)
                    else metadata["show_emojis"]
                    if isinstance(metadata.get("show_emojis"), bool)
                    else task.get("show_emojis")
                ),
                cleanup_settings=cleanup_settings,
            ),
        )

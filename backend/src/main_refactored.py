    ...
    *,
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            if os.getenv("PUBLISH_SCHEDULER_ENABLED", "true").lower() != "false":
                ...
                async def _publish_scheduler_loop() -> None:
                    logger.info(
                        "▶️  Publish scheduler started (every %ss)", SCHEDULER_INTERVAL_SECONDS
                    )
                    while True:
                        try:
                            async with AsyncSessionLocal() as db:
                                await run_publish_schedule_once(db)
                                await run_tiktok_publish_schedule_once(db)
                        except Exception:
                            logger.exception("Publish scheduler run failed")
                        await asyncio.sleep(SCHEDULER_INTERVAL_SECONDS)

                scheduler_task = asyncio.create_task(_publish_scheduler_loop())
    ...
    # Google OAuth redirects go straight to the backend's /api/publish/youtube/callback.

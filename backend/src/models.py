...
class Profile(Base):
    """A publishing profile (channel) owned by a user.

    Profiles group connected accounts (YouTube/TikTok) so a single user can
    publish to multiple channels. Tasks stay per-user; each profile simply
    bundles the OAuth accounts used for publishing.
    """
...
class ConnectedAccount(Base):
    """A platform account (YouTube/TikTok) connected to a publishing profile.

    One account per platform per profile. Secrets are stored encrypted as a
    JSON object (``token_data``) reusing the app_settings encryption; display
    metadata (channel/account name, avatar, etc.) is stored plain in
    ``display_data`` so the UI can list accounts without decrypting tokens.
    """
    ...
    display_data: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
    ...
    __table_args__ = (
        UniqueConstraint("profile_id", "platform", name="uq_connected_accounts_profile_platform"),
    )
...
class GeneratedClip(Base):
    ...
    publish_status: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default=sql_text("'none'")
    )
    ...
    # Profile used when this clip was marked for publishing. Lets the scheduler
    # resolve the correct connected account per clip instead of using a single
    # global channel.
    publish_profile_id: Mapped[Optional[str]] = mapped_column(
    ...
    # TikTok auto-publishing (self-hosted schedule; TikTok has no native
    # scheduling, so the worker publishes each clip when its slot time hits).
    tiktok_publish_requested: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default=sql_text("false")
    )
    tiktok_publish_status: Mapped[str] = mapped_column(
        String(20), nullable=False, server_default=sql_text("'none'")
    )
    tiktok_scheduled_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    tiktok_published_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    tiktok_publish_error: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

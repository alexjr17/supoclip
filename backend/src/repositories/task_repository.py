...
def _urls_equivalent(url_a: str, url_b: str) -> bool:
    """Compare URLs for identity.

    Falls back to exact string match when no provider-specific identity
    (e.g. a YouTube video ID) can be extracted from either URL.
    """
    if not isinstance(url_a, str) or not isinstance(url_b, str):
        return False

    try:
        from ...youtube_utils import get_youtube_video_id

        video_a = get_youtube_video_id(url_a)
        video_b = get_youtube_video_id(url_b)
...
class TaskRepository:
    ...
    @staticmethod
    async def find_previous_source_usage(
        db: AsyncSession, user_id: str, source_url: str
    ) -> Optional[Dict[str, Any]]:
        """Return the most recent task for this user that used the same source URL.

        Only meaningful for URLs where a stable identity can be derived (e.g.
        YouTube video IDs). Returns None when the URL was never used before.
        """
        result = await db.execute(
            text(
                """
                SELECT t.id, t.status, t.created_at, s.url AS source_url,
        ...
        for row in result.fetchall():
            previous_url = getattr(row, "source_url", None)
            if previous_url and _urls_equivalent(previous_url, source_url):
                return {
                    "task_id": row.id,

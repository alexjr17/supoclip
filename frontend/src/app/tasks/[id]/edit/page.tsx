...
export default function TaskEditPage() {
  ...
  const emojiByIndex = useMemo(
    () => annotateCaptionWords(subtitleWords, { enableEmoji: projectShowEmojis }),
    [subtitleWords, projectShowEmojis]
  );
  ...
  const activeSubtitleSegments = useMemo(() => {
    ...
    return subtitleWords.slice(start, start + 6).map((word, index) => ({
      word,
      emoji: emojiByIndex.get(start + index) ?? "",
    }));
  }, [currentTime, selectedClip?.duration, subtitleWords, emojiByIndex]);

  const getSubtitleSegmentsAtTime = useCallback(
    (timeSeconds: number, durationSeconds: number) => {
      if (subtitleWords.length === 0) return [] as { word: string; emoji: string }[];
      ...
      return subtitleWords.slice(startIndex, startIndex + 6).map((word, index) => ({
        word,
        emoji: emojiByIndex.get(startIndex + index) ?? "",
      }));
    },
    [subtitleWords, emojiByIndex]
  );
  ...
  useEffect(() => {
    ...
    setSubtitleY(78);
    setSubtitleSize(typeof projectFontSize === "number" && projectFontSize > 0 ? projectFontSize : 52);
  }, [selectedClip, projectFontSize]);
  ...
  const handleSplit = async (splitAt?: number) => {
    ...
    const minSplit = MIN_GAP_SECONDS;
    const maxSplit = Math.max(selectedClip.duration - MIN_GAP_SECONDS, minSplit);
    const clamped = clamp(value, minSplit, maxSplit);
    await withSaving(async () => {
      const response = await fetch(`${taskApiUrl}/${task.id}/clips/${selectedClip.id}/split`, {
        ...
        body: JSON.stringify({ split_time: Number(clamped.toFixed(2)) }),
      });
      ...
      const conversion = await Conversion.init({
        ...
        video: {
          ...
          process: (sample) => {
            ...
            const subtitleAtTime = getSubtitleSegmentsAtTime(browserSample.timestamp, targetDuration);
            if (projectAddSubtitles && subtitleAtTime.length > 0) {
              const fontSize = Math.max(24, Math.round(subtitleSize));
              const captionColor = projectFontColor ?? "#ffffff";
              ctx.font = `700 ${fontSize}px ui-sans-serif, system-ui, "Segoe UI Emoji", "Apple Color Emoji", "Noto Color Emoji", sans-serif`;
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";

              const text = subtitleAtTime.map((segment) => `${segment.word}${segment.emoji}`).join(" ");
              ...
              for (const segment of subtitleAtTime) {
                const cleanedWord = segment.word.toLowerCase().replace(/[^a-z0-9']/g, "");
                ctx.fillStyle = highlightSet.has(cleanedWord) ? "#fde047" : captionColor;
                const wordWidth = ctx.measureText(segment.word).width;
                ctx.fillText(segment.word, cursorX + wordWidth / 2, y);
                cursorX += wordWidth;
                if (segment.emoji) {
                  const emojiWidth = ctx.measureText(segment.emoji).width;
                  ctx.fillText(segment.emoji, cursorX + emojiWidth / 2, y);
                  cursorX += emojiWidth;
                }
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            ...
                            display: projectAddSubtitles ? undefined : "none",
                          }}
                        >
                          {activeSubtitleSegments.length > 0 ? (
                            activeSubtitleSegments.map((segment, index) => {
                              const cleaned = segment.word.toLowerCase().replace(/[^a-z0-9']/g, "");
                              const highlighted = highlightWords.includes(cleaned);
                              return (
                                <span key={`${segment.word}-${index}`} className={highlighted ? "text-yellow-300" : "text-white"}>
                                  {segment.word}
                                  {segment.emoji}
                                  {index === activeSubtitleSegments.length - 1 ? "" : " "}
                                </span>
                              );
                            })
                          ) : (

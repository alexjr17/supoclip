...
interface Clip {
  ...
  tiktok_publish_requested: boolean;
...
export default function TaskPage() {
  ...
  const [publishBusy, setPublishBusy] = useState<"mass" | "sync" | "today" | null>(null);
  const [tiktokBusy, setTiktokBusy] = useState<"mass" | "sync" | "today" | null>(null);
  ...
  const [projectAddSubtitles, setProjectAddSubtitles] = useState(true);
  const [projectShowEmojis, setProjectShowEmojis] = useState(true);
  ...
  const [availableTemplates, setAvailableTemplates] = useState<
    Array<{ id: string; name: string; description: string; animation: string; emoji?: boolean }>
  >([]);
  ...
  const fetchTaskStatus = useCallback(
    async (retryCount = 0, maxRetries = 5) => {
      ...
      try {
        ...
        setProjectAddSubtitles(taskData.add_subtitles === null || taskData.add_subtitles === undefined ? true : Boolean(taskData.add_subtitles));
        setProjectShowEmojis(taskData.show_emojis === null || taskData.show_emojis === undefined ? true : Boolean(taskData.show_emojis));
    ...
    const loadYoutubeStatus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setYoutubeConfigured(Boolean(data.youtube_configured));
          setTiktokConfigured(Boolean(data.tiktok_configured));
        }
      } catch (err) {
  ...
  const handleTogglePublish = async (clipId: string) => {
    if (!session?.user?.id || !params.id) return;

    const clip = clips.find((c) => c.id === clipId);
    if (!clip) return;
    const nextValue = !clip.publish_requested;

    try {
      const response = await fetch(
        `${taskApiUrl}/${params.id}/clips/publish`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ clip_ids: [clipId], publish: nextValue }),
        },
      );

      if (!response.ok) {
        alert(await buildSupportError(response, "Failed to update publish setting"));
        return;
      }
      ...
      setClips((prev) =>
        prev.map((c) =>
          c.id === clipId
            ? {
                ...c,
                publish_requested: nextValue,
                publish_status: nextValue ? "pending" : "none",
                scheduled_publish_at: nextValue ? c.scheduled_publish_at : null,
                publish_error: nextValue ? c.publish_error : null,
              }
            : c,
        ),
      );
      ...
      alert(err instanceof Error ? err.message : "Failed to update publish setting");
    }
  };

  const handleToggleTikTokPublish = async (clipId: string) => {
    if (!session?.user?.id || !params.id) return;

    const clip = clips.find((c) => c.id === clipId);
    if (!clip) return;
    const nextValue = !clip.tiktok_publish_requested;

    try {
      const response = await fetch(
        `${taskApiUrl}/${params.id}/clips/tiktok-publish`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ clip_ids: [clipId], publish: nextValue }),
      ...
      if (!response.ok) {
        alert(await buildSupportError(response, "Failed to update TikTok publish setting"));
        return;
      }
      ...
      setClips((prev) =>
        prev.map((c) =>
          c.id === clipId
            ? {
                ...c,
                tiktok_publish_requested: nextValue,
                tiktok_publish_status: nextValue ? "pending" : "none",
                tiktok_scheduled_at: nextValue ? c.tiktok_scheduled_at : null,
                tiktok_publish_error: nextValue ? c.tiktok_publish_error : null,
              }
            : c,
        ),
      );
    } catch (err) {
      console.error("Error toggling TikTok publish:", err);
      alert(err instanceof Error ? err.message : "Failed to update TikTok publish setting");
    }
  };

  const runTikTokPublishAction = async (
    action: "mass" | "sync" | "today",
    ...
    try {
      const response = await fetch(
        `/api/publish/tiktok/${action === "mass" ? "schedule" : action === "today" ? "today" : "sync"}`,
        { method: "POST" },
      );
      if (!response.ok) {
        throw new Error(await buildSupportError(response, "TikTok publish action failed"));
      }
      const data = await response.json();
      let message = successMessage;
      if (data.published_now !== undefined) {
        message = `${data.published_now} clip${data.published_now === 1 ? "" : "s"} published now.`;
        if (data.scheduled > 0) {
          message += ` ${data.scheduled} scheduled for later.`;
        }
      } else if (data.published !== undefined) {
        message = `${data.published} clip${data.published === 1 ? "" : "s"} published now.`;
        if (data.skipped > 0) {
          message += ` ${data.skipped} skipped (daily limit ${data.daily_limit} reached).`;
        }
      } else if (data.matches && data.matches.length > 0) {
        message = `${successMessage} ${data.matches.length} clip${data.matches.length === 1 ? "" : "s"} already matched on TikTok.`;
      }
      alert(message);
      await fetchTaskStatus();
    } catch (err) {
      console.error("Error in TikTok publish action:", err);
      alert(err instanceof Error ? err.message : "TikTok publish action failed");
    } finally {
      setTiktokBusy(null);
    }
  };
  ...
  const runPublishAction = async (
    ...
    if (!session?.user?.id || !params.id) return;
    setPublishBusy(action);
    try {
      const response = await fetch(
        `/api/publish/${action === "mass" ? "mass-upload" : action === "today" ? "today" : "sync"}`,
        { method: "POST" },
      );
      if (!response.ok) {
        throw new Error(await buildSupportError(response, "YouTube publish action failed"));
      }
      const data = await response.json();
      let message = successMessage;
      if (data.published !== undefined) {
        ...
        const uploaded = data.uploaded ?? data.scheduled;
        message = `${uploaded} clip${uploaded === 1 ? "" : "s"} uploaded and scheduled.`;
        if (data.already_published > 0) {
          message += ` ${data.already_published} already published.`;
        }
        if (data.failed > 0) {
          const resetAt = data.quota_reset_at ? new Date(data.quota_reset_at) : null;
          const resetLabel =
            resetAt && !isNaN(resetAt.getTime())
              ? ` Quota resets at ${resetAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} local time.`
              : "";
          message += `\n${data.failed} failed${data.quota_blocked ? " (YouTube daily upload quota)" : ""}.${resetLabel}`;
          const reasons = [
            ...new Set(
              ((data.failures || []) as { error?: string }[])
                .map((f) => f.error)
          ...
          if (reasons.length > 3) message += `\n- +${reasons.length - 3} more`;
        }
        if (data.deferred > 0) {
          message += `\n${data.deferred} deferred (will retry later).`;
        }
      } else if (data.matches && data.matches.length > 0) {
        message = `${data.matches.length} clip${data.matches.length === 1 ? "" : "s"} reconciled with YouTube.`;
        if (data.already_recorded > 0) {
          message += ` ${data.already_recorded} already recorded.`;
        }
      }
      alert(message);
      await fetchTaskStatus();
    } catch (err) {
      console.error("Error in YouTube publish action:", err);
      alert(err instanceof Error ? err.message : "YouTube publish action failed");
    } finally {
      setPublishBusy(null);
    }
  };
    ...
    try {
      const response = await fetch(`${taskApiUrl}/${params.id}/settings`, {
        ...
        body: JSON.stringify({
          ...
          add_subtitles: projectAddSubtitles,
          show_emojis: projectShowEmojis,
  ...
  const handleProjectTemplateChange = (templateId: string) => {
    setProjectCaptionTemplate(templateId);
    const template = availableTemplates.find((t) => t.id === templateId);
    if (template && typeof template.emoji === "boolean") {
      setProjectShowEmojis(template.emoji);
    }
  };
          ...
          {task && (
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                ...
                {task.status === "completed" && clips.length > 0 && youtubeConfigured && (
                  <>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void runPublishAction("mass", "All clips sent to YouTube.")}
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void runPublishAction("today", "Today's clips published.")}
                      disabled={publishBusy !== null}
                      className="text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700"
                    >
                      {publishBusy === "today" ? (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void runPublishAction("sync", "YouTube sync complete.")}
                      disabled={publishBusy !== null}
                    >
                      {publishBusy === "sync" ? (
                        <RefreshCw className="w-4 h-4 animate-spin" />
                ...
                {task.status === "completed" && clips.length > 0 && tiktokConfigured && (
                  <>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void runTikTokPublishAction("mass", "TikTok clips scheduled.")}
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void runTikTokPublishAction("today", "Today's TikTok clips published.")}
                      disabled={tiktokBusy !== null}
                      className="text-stone-700 border-stone-300 hover:bg-stone-100"
                    >
                      {tiktokBusy === "today" ? (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => void runTikTokPublishAction("sync", "TikTok sync complete.")}
                      disabled={tiktokBusy !== null}
                      className="text-stone-700 border-stone-300 hover:bg-stone-100"
                    >
                      {tiktokBusy === "sync" ? (
                        ...
                        <div className="p-6 flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-3">
                            ...
                            {task?.status === "completed" && (
                              <Button
                                size="sm"
                                variant={clip.publish_requested ? "default" : "outline"}
                                onClick={() => handleTogglePublish(clip.id)}
                                disabled={clip.publish_status === "published"}
                                title={
                                  clip.publish_status === "published"
                                    ? "Already published"
                                    : clip.publish_requested
                                      ? "Remove from publish queue"
                                      : "Add to automatic YouTube publish queue"
                                }
                              >
                                <Clapperboard className="w-4 h-4" />
                                {clip.publish_status === "published"
                            ...
                            {task?.status === "completed" && tiktokConfigured && (
                              <Button
                                size="sm"
                                variant={clip.tiktok_publish_requested ? "default" : "outline"}
                                onClick={() => void handleToggleTikTokPublish(clip.id)}
                                disabled={clip.tiktok_publish_status === "published"}
                                title={
                                  clip.tiktok_publish_status === "published"
                                    ? "Already published to TikTok"
                                    : clip.tiktok_publish_requested
                                      ? "Remove from TikTok publish queue"
                                      : "Add to scheduled TikTok publish queue"
                                }
                              >
                                <Music2 className="w-4 h-4" />
                                {clip.tiktok_publish_status === "published"
                            ...
                            {clip.publish_status === "scheduled" && clip.scheduled_publish_at && (
                              <Badge variant="secondary" className="text-xs">
                                <Clock className="w-3 h-3 mr-1" />
                                {new Date(clip.scheduled_publish_at).toLocaleString()}
                              </Badge>
                            )}
                            {clip.tiktok_publish_status === "scheduled" && clip.tiktok_scheduled_at && (
                              <Badge variant="secondary" className="text-xs">
                                <Music2 className="w-3 h-3 mr-1" />
                                TikTok {new Date(clip.tiktok_scheduled_at).toLocaleString()}
                              </Badge>
                            )}
                            ...
                            {clip.tiktok_publish_status === "failed" && clip.tiktok_publish_error && (
                              <Badge variant="destructive" className="text-xs">
                                <AlertCircle className="w-3 h-3 mr-1" />
              ...
              <SheetContent side="right" className="sm:max-w-md overflow-y-auto">
                ...
                <div className="space-y-5 px-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={projectCaptionTemplate} onValueChange={handleProjectTemplateChange}>
                      <SelectTrigger>
                        <SelectValue>
                  ...
                  <div className="rounded-lg border bg-gray-50 p-3 space-y-3">
                    <div>
                      <div className="text-sm font-medium text-gray-900">Captions</div>
                      <div className="text-xs text-gray-500">Toggle caption rendering for regenerated clips.</div>
                    </div>

                    <label className="flex items-center gap-2 text-sm text-gray-700">
                      <input
                        type="checkbox"
                        checked={projectAddSubtitles}
                        onChange={(e) => setProjectAddSubtitles(e.target.checked)}
                        className="rounded"
                      />
                      Subtitles
                    </label>
                    ...
                    <label className="flex items-center gap-2 text-sm text-gray-700">
                      <input
                        type="checkbox"
                        checked={projectShowEmojis}
                        onChange={(e) => setProjectShowEmojis(e.target.checked)}
                        className="rounded"
                      />
                      Emojis
                    </label>

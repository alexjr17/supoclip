...
export default function ListPage() {
  ...
  const runPublishAction = async (
    ...
    try {
      ...
      setPublishNotice({ tone: "success", message });
      await refreshTasks();
    } catch (err) {
      console.error("Error in YouTube publish action:", err);
      setPublishNotice({
        tone: "error",
        message: err instanceof Error ? err.message : "YouTube publish action failed",
      });
    } finally {
      setPublishBusy(null);
    }
  ...
  const runPublishToTikTok = async (action: "mass" | "sync" | "today" = "mass") => {
    if (!session?.user?.id) return;
    ...
    try {
      ...
      const data = await response.json();
      let message = `${data.published_now ?? data.published ?? 0} clip${(data.published_now ?? data.published ?? 0) === 1 ? "" : "s"} published to TikTok now.`;
      if (data.scheduled > 0) {
        message += ` ${data.scheduled} scheduled for later.`;
      }
      if (data.matches && data.matches.length > 0) {
        message = `${data.matches.length} clip${data.matches.length === 1 ? "" : "s"} already matched on TikTok.`;
      }
      if (data.published_now === 0 && data.published === 0 && data.scheduled === 0 && !data.matches) {
        message = "No clips to schedule for TikTok right now.";
      }
      setPublishNotice({ tone: "success", message });
      await refreshTasks();
    } catch (err) {
      ...
      setPublishNotice({
        tone: "error",
        message: err instanceof Error ? err.message : "TikTok publish action failed",
      });
    } finally {
      setTiktokBusy(null);
    }
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {tiktokConfigured ? (
                  <>
                    <Button
                      size="sm"
                      onClick={() => void runPublishToTikTok("mass")}
                      disabled={tiktokBusy !== null || publishBusy !== null}
                      className="border-stone-300 text-stone-700 hover:bg-stone-100"
                      variant="outline"
                    >
                      {tiktokBusy === "mass" ? (
                    ...
                    <Button
                      size="sm"
                      onClick={() => void runPublishToTikTok("today")}
                      disabled={tiktokBusy !== null || publishBusy !== null}
                      className="border-stone-300 text-stone-700 hover:bg-stone-100"
                      variant="outline"
                    >
                    ...
                    <Button
                      size="sm"
                      onClick={() => void runPublishToTikTok("sync")}
                      disabled={tiktokBusy !== null || publishBusy !== null}
                      className="border-stone-300 text-stone-700 hover:bg-stone-100"
                      variant="outline"
                    >

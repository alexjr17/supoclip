  ...
  if (!uploadResponse.ok) {
    const fallbackMessage =
      uploadResponse.status === 413
        ? "Uploaded file is too large. Please upload a video under 5 GB."
...
export default function HomeApp() {
  ...
  const [availableTemplates, setAvailableTemplates] = useState<Array<{ id: string, name: string, description: string, animation: string, font_family?: string, font_size?: number, font_color?: string, emoji?: boolean }>>([]);
  ...
  const [isLoadingLatest, setIsLoadingLatest] = useState(false);  const [billingSummary, setBillingSummary] = useState<BillingSummary | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const warning = startResult.warning;
      if (warning?.code === "SOURCE_ALREADY_USED") {
        setSourceWarning({
          taskId: taskIdFromStart,
          message: warning.message,
          previousTitle: warning.previous_usage?.source_title,
          previousCreatedAt: warning.previous_usage?.created_at,
        });
        return;
      }
                ...
                <CardContent className="px-4 pt-0 pb-2.5 space-y-2.5">
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-lg bg-stone-50">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-medium text-stone-900">Emojis</h3>
                        <p className="text-xs text-stone-500">Add context emojis next to keywords in captions</p>
                      </div>
                    </div>
                    <Switch
                      checked={showEmojis}
